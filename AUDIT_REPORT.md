...
## 6. Dodatečná revize a dorovnání oprav

Po kontrole implementovaných změn byly nalezeny a doplněny tyto navazující opravy:

- `listDashboardSubApps_()` už nepoužívá neexistující `auth.user`; bere předpočítaná oprávnění z `auth.subApps`.
- Běžní uživatelé vidí jen aktivní dlaždice, ke kterým mají záznam v `SUBAPP_PERMISSIONS`; admin role dál vidí všechny aktivní dlaždice včetně stavu `PREPARING`.
- `ensureDatabase_()` ošetřuje neplatné ID i v cache větvi a maže vadnou cache před recovery.
- Schéma databáze je řízené přes `DATABASE_SCHEMA_VERSION`; migrace se spustí při chybějící nebo starší verzi, ne při každém běžném requestu.
- `deleteLocation()` kontroluje nejen vazby z `DEPARTMENTS.locationIds`, ale i uživatele v `USERS.locationName`, aby po smazání nezůstaly osiřelé reference.
